...
class SimpleCpu:
    ...
    def evaluateInstruction(self, str):
        strList = str.split(" ")

        if(strList[5] == ">"):
            comp = self.checkAndGet(strList[4])
            if  not (comp > int(strList[6])):
                return
        elif(strList[5] == "<"):
            comp = self.checkAndGet(strList[4])
            if  not (comp < int(strList[6])):
        ...
        if(strList[1] == "dec"):
            self.checkAndAdd(strList[0], -1 * int(strList[2]))
        elif(strList[1] == "inc"):
            self.checkAndAdd(strList[0], int(strList[2]))
        else:
            print("Invalid instruction type.")
            pass

    def checkAndAdd(self, reg, offset):
        if self.registers.has_key(reg):
            self.registers[reg] += offset
            self.CheckIfLargest(self.registers[reg])
        else:
            self.registers.update({reg: offset})

    def checkAndGet(self, reg):
        if self.registers.has_key(reg):
            return self.registers[reg]
        else:
            self.registers.update({reg: 0})
            return 0

    def CollectInstructions(self, fileName):
        insFile = open(fileName, "r")
        input = insFile.read()
        self.inputArr = input.split("\n")
        
    
    def Compute(self):
        for str in self.inputArr:
            self.evaluateInstruction(str)
        
    def PrintRegisters(self):
        print self.registers
        print self.largestNum

    def CheckIfLargest(self, val):
        if self.largestNum < val:
            self.largestNum = val

x = SimpleCpu()

x.CollectInstructions("input.dat")
x.Compute()
x.PrintRegisters()