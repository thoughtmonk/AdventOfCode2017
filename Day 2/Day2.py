...
def getLargestDifference(str):
    numberArray = str.split("\t")
    minValue = maxValue = int(numberArray[0])
...
def getRowDivisor(str):
   strArray = str.split("\t")
   numberArray = [int(str) for str in strArray]
   numberArray.sort()
   startLocation = 1

   for i in numberArray:
       for j in reversed(numberArray):
           if ((j % i) == 0) and i != j:
               return j / i
...
for line in inputFile:
    captcha1 += getLargestDifference(line)
    captcha2 += getRowDivisor(line)
