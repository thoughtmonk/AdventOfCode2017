...
At this point, we've reached a state we've seen before: 2 4 1 2 was already seen. The infinite loop is detected after the 
fifth block redistribution cycle, and so the answer in this example is 5.
...
def redistributeMemory(arr):
    ...
    while maxValue > 0:
        maxLocation = (maxLocation + 1) % len(arr)
        arr[maxLocation] += 1
        maxValue -= 1
...
array = file.read()
array = array.split("\t")
memoryArr = map(int, array)
initialArr = deepcopy(memoryArr)

previousAlloc = []
previousAlloc.append(deepcopy(memoryArr))

numberOfRedist = 0

while (1):
    redistributeMemory(memoryArr)
    numberOfRedist += 1
    if memoryArr in previousAlloc:
        previousAlloc.append(deepcopy(memoryArr))
        break
    
    previousAlloc.append(deepcopy(memoryArr))

print(numberOfRedist)
location = 0

for mem in previousAlloc:
    if mem == previousAlloc[-1]:
        print(len(previousAlloc) - location)
        break
    location += 1