...
def evaluateGroup(arr, val):
    ...
    while arr[loc] != "}":
        ...
        if arr[loc] == "!":
            ...
            loc += evaluateGarbage(arr[loc+1:])
        elif arr[loc] == "{":
            newVals = evaluateGroup(arr[loc:], val + 1)
            value += newVals[0]
            loc += 1 + newVals[1]
...
testString5 = "{<{},{},{{}}>}"
testString6 = "{<a>,<a>,<a>,<a>}"
testString7 = "{{<ab>},{<ab>},{<ab>},{<ab>}}"
testString8 = "{{<!!>},{<!!>},{<!!>},{<!!>}}"
testString9 = "{{<a!>},{<a!>},{<a!>},{<ab>}}"

print(evaluateGroup(testString9, 1)[0])
...
while loc < len(strArr):
    if strArr[loc] == "!":
        loc += 2
        continue;
    elif strArr[loc] == "<":
        loc += evaluateGarbage(strArr[loc+1:])
    elif strArr[loc] == "{":
        newVals = evaluateGroup(strArr[loc:], 1)
        loc += newVals[1]
        val += newVals[0]
    else:
        loc += 1
