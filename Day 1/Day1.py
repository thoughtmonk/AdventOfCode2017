...
a human. Apparently, you only get one millisecond to solve the captcha: too fast for a 
...
def getCaptcha(str):
    ...
    for c in str:
        if(previousValue == c):
            totalValue += int(previousValue)
        previousValue = c

    if(str[0] == str[len(str)-1]):
        totalValue += int(str[0])

    return totalValue

def getCaptchaLoc(str):
    totalValue = 0
    location = len(str) / 2

    for c in str:
        cmpValue = str[location]
        if(c == cmpValue):
            totalValue += int(cmpValue)
        
        location = (location + 1) % len(str)
...
print(getCaptcha(inputStr))
print(getCaptchaLoc(inputStr))